...
class FrontController extends Controller
{
    ...
    public function createUser(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($emailexists == "") {
                ...
                $mfrom = $this->admin->id;
                $mto = $id;
                $message = "Account created. Your Email: " . $email . "and Password: " . $pass;
    ...
    public function createRorder(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $mfrom = $admin->id;
            $mto = Auth::user()->id;
            $message = "You have placed an order with id # " . $id . ". Our writers are reviewing it. Please check bids and assign.";
        ...
        if ($validator->fails()) {
            ...
            if ($request->title == null) {
                ...
                if ($user != null) {
                    ...
                    $mfrom = $admin->id;
                    $mto = $id;
                    $message = "Thank you for choosing researchpaperhelper.com as your custom paper provider. You have completed registration on ResearchPaperHelper. You account has been created. Your email is " . $email . " and your password is " . $pass . ". Change password on login. ";
                    $msubject = "CONGRATULATIONS! Welcome to ResearchPaperHelper.com";
                    $datecreated = Carbon::now();
                    $user_id = $id;
                    app()->call('App\Http\Controllers\SendemailsController@prepareEmail', [$mfrom, $mto, $msubject, $message, "", $datecreated, $user_id]);
                    ...
                    $mfrom = $admin->id;
                    $mto = $id;
                    $message = "You have placed an order with ID# " . $oid . ". Our writers are reviewing it. Please check for bids.";
                    $msubject = "Order Placed";
                    $datecreated = Carbon::now();
                    $user_id = $id;
                    app()->call('App\Http\Controllers\SendemailsController@prepareEmail', [$mfrom, $mto, $msubject, $message, "", $datecreated, $user_id]);
                    ...
                    if (Auth::loginUsingId($userE->id)) {
                        flash('Success! Order submitted. Please update this information. Check your e-mail for logins ', 'success');
