@if($mybid=="")
	@if((Auth::user()->role=="1"||Auth::user()->role=="2"||Auth::user()->role=="4")&&$order->status==1)
	<a data-toggle="modal" data-target="#placebid" class="btn btn-primary">Place Bid</a>
	@endif
	<div id="placebid" class="modal" role="dialog" data-backdrop="static" data-keyboard="false">
					...
					<form class="form-horizontal" role="form"  method="post" action="{{ url('/place-bid') }}">
						...
						<div class="{{ $errors->has('amount') ? ' has-error' : '' }}">
							...
							<div class="input-group">
							    <input id="amount" type="number" min="1"  class="form-control" name="amount" value="{{ old('amount') }}" oninput="this.value=this.value.replace(/[^0-9]/g,'');" required="true">
							</div>
						...
						<div class="{{ $errors->has('offer') ? ' has-error' : '' }}">
							...
							<textarea    class="form-control " name="offer" value="{{ old('offer') }}" rows="3" maxlength="300" required="true"></textarea>
							@if ($errors->has('offer'))
